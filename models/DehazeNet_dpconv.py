...
class DRB(nn.Module):  # 尺寸没变
    def __init__(self, in_channels=64, growth_rate=32):   #in_channels=64, growth_rate=32
        ...
        self.depthwise = nn.Conv2d(192, 192, kernel_size=3, stride=1, padding=1, groups=growth_rate)
...
class DehazeNet(nn.Module):
    ...
    def forward(self, x):
        pre = self.relu1(self.down1(self.conv1(x)))

        x1 = self.drb1(pre)
        x2 = self.drb2(x1)
        x3 = self.attention1(x2)
        block1 = x1 + x2 + x3
        block1_out = self.block_relu1(self.block_conv1(block1)) + pre

        x4 = self.drb3(block1_out)
        x5 = self.drb4(x4)
        x6 = self.attention2(x5)
        block2 = x4 + x5 + x6
        block2_out = self.block_relu2(self.block_conv2(block2)) + block1_out

        x7 = self.drb5(block2_out)
        x6 = self.drb6(x7)
        x9 = self.attention3(x6)
        block3 = x7 + x6 + x9
        block3_out = self.block_relu3(self.block_conv3(block3)) + block2_out

        x10 = self.drb7(block3_out)
        x11 = self.drb8(x10)
        x12 = self.attention4(x11)
        block4 = x10 + x11 + x12
        block4_out = self.block_relu4(self.block_conv4(block4)) + block3_out

        block4_out = block4_out + pre
        out1 = self.conv2(block4_out)
        out1_up = F.interpolate(out1, scale_factor=2, mode='bilinear', align_corners=False)
        out1 = self.tanh1(out1_up)

        # -----第一阶段完成----- #

        fe = self.fe(x, out1)

        fe = torch.relu(self.down2(self.conv3(fe)))

        details_block1 = self.detail2(self.detail1(fe))
        ...
        details_out = details_block4 + fe

        out2 = self.conv4(details_out)
