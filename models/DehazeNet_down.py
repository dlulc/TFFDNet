...
class DehazeNet(nn.Module):
    def __init__(self):
        ...
        self.conv4 = nn.Conv2d(in_channels=3, out_channels=64, kernel_size=1)
    ...
    def forward(self, x):
        ...
        x7 = self.drb7(x6)
        x8 = self.drb8(x7)
        x9 = self.attention3(self.drb9(x8)) + pre

        x9_up = F.interpolate(x9, scale_factor=2, mode='bilinear', align_corners=False)
        out1 = self.tanh1(self.conv2(x9_up))

        fe = self.fe(x, out1)
        fe = self.down2(fe)
        fe = torch.relu(self.conv4(fe))
        ...
        fusion3_up = F.interpolate(fusion3, scale_factor=2, mode='bilinear', align_corners=False)
        out = self.tanh2(self.conv3(fusion3_up))
