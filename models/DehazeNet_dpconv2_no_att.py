...
class DehazeNet(nn.Module):
    ...
    def forward(self, x):
        ...
        block1_out = self.block_relu1(self.block_conv1(x2)) + pre

        x4 = self.drb3(block1_out)
        x5 = self.drb4(x4)
        # x6 = self.attention2(x5) + x5
        block2_out = self.block_relu2(self.block_conv2(x5)) + block1_out

        x7 = self.drb5(block2_out)
        x6 = self.drb6(x7)
        # x9 = self.attention3(x6) + x6
        block3_out = self.block_relu3(self.block_conv3(x6)) + block2_out

        x10 = self.drb7(block3_out)
        x11 = self.drb8(x10)
        # x12 = self.attention4(x11) + x11
        block4_out = self.block_relu4(self.block_conv4(x11)) + block3_out
